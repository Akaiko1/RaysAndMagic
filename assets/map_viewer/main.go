...
func legendItemCount(groups []legendGroup) int {
	count := 0
...
func appendLegendScope(out []legendEntry, label, collapseID string, groups []legendGroup, collapsed map[string]bool) []legendEntry {
	...
	out = append(out, collapsibleHeader(fmt.Sprintf("%s (%d)", label, legendItemCount(groups)), collapseID, scopeCollapsed, 0))
	if scopeCollapsed {
		return out
	}
	for _, group := range groups {
...
func appendLegendGroup(out []legendEntry, group legendGroup, collapsed map[string]bool, depth int) []legendEntry {
	...
	out = append(out, collapsibleHeader(
		fmt.Sprintf("%s (%d)", group.label, legendItemCount([]legendGroup{group})),
		group.id,
		groupCollapsed,
		depth,
	))
...
func humanizePaletteName(value string) string {
...
func buildLegendEntries(tm *world.TileManager, mc *monster.MonsterYAMLConfig, biome string, collapsed map[string]bool) []legendEntry {
	...
	entries = append(entries, collapsibleHeader("Tools (1)", toolsCollapseID, toolsCollapsed, 0))
	if !toolsCollapsed {
	...
	if len(biomeGroups) > 0 {
		biomeLabel := humanizePaletteName(biome)
		if biomeLabel == "" {
	...
	if character.NPCConfigInstance != nil && len(character.NPCConfigInstance.NPCs) > 0 {
		...
		for _, cat := range cats {
			...
			catLabel := humanizePaletteName(cat)
			if catLabel == "" {
				catLabel = "Other"
			}
	...
	entries = append(entries, collapsibleHeader("Notes (3)", notesCollapseID, notesCollapsed, 0))
	if !notesCollapsed {
