...
func (w *World3D) isSuitableLocationFromConfig(x, y float64, monsterKey string) bool {
	...
	if !w.isTileWalkable(currentTile) {
		return false
	}
...
// Helper methods
func (w *World3D) isTileWalkable(tile TileType3D) bool {
	...
	switch tile {
	case TileEmpty, TileClearing, TileMushroomRing, TileFireflySwarm, TileFernPatch:
		return true
	default:
		return false
	}
