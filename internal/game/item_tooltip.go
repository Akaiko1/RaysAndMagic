...
// Legacy function for backward compatibility - needs combat system access
func GetItemDamageTooltip(item items.Item, char *character.MMCharacter) string {
	// This function needs access to combat system for real calculations
	// For now, return basic tooltip without combat system
	return GetItemTooltipLegacy(item, char)
}

// GetItemTooltipLegacy provides basic tooltip without combat system access
func GetItemTooltipLegacy(item items.Item, char *character.MMCharacter) string {
	var tooltip []string
	...
	// For weapons, show basic damage info
	if item.Type == items.ItemWeapon {
		...
		tooltip = append(tooltip, fmt.Sprintf("Base Damage: %d", base))
		tooltip = append(tooltip, fmt.Sprintf("Might Bonus: +%d", bonus))
		tooltip = append(tooltip, fmt.Sprintf("Total Damage: %d", totalDamage))

		if item.Range > 0 {
			tooltip = append(tooltip, fmt.Sprintf("Range: %d tiles", item.Range))
		}
	}
...
func getWeaponTooltip(item items.Item, char *character.MMCharacter, combatSystem *CombatSystem) []string {
	...
	lines = append(lines, fmt.Sprintf("%s Bonus: +%d", bonusStatName, bonus))
...
func GetSpellTooltip(spellID spells.SpellID, char *character.MMCharacter, combatSystem *CombatSystem) string {
	...
	if magicSkill, exists := char.MagicSchools[school]; exists {
		...
		spellType := spells.SpellIDToType(spellID)
		effectiveness := calculateSpellEffectivenessFromType(spellType, magicSkill)
	...
	spellType := spells.SpellIDToType(spellID)
	spellDetails := getSpellMechanicsFromDefinition(def, char, combatSystem, spellType)
...
// calculateSpellEffectivenessFromType calculates spell effectiveness using SpellType
func calculateSpellEffectivenessFromType(spellType spells.SpellType, skill *character.MagicSkill) int {
	// Base effectiveness depends on spell complexity
	def := spells.GetSpellDefinition(spellType)
...
func getSpellMechanicsFromDefinition(def spells.SpellDefinition, char *character.MMCharacter, combatSystem *CombatSystem, spellType spells.SpellType) []string {
	var details []string
	...
	if def.School == "fire" || def.School == "air" || def.School == "water" || def.School == "earth" {
		...
		baseDamage, intellectBonus, totalDamage := spells.CalculateSpellDamage(spellType, effectiveIntellect)
	...
	if def.School == "body" || (def.Name == "First Aid" || def.Name == "Heal") {
		...
		baseHeal, personalityBonus, totalHeal := spells.CalculateHealingAmount(spellType, effectivePersonality)
