...
func (rh *RenderingHelper) CreateTexturedWallSlice(tileType world.TileType3D, distance float64, width, height, wallSide int, textureCoord float64) *ebiten.Image {
	...
	// Calculate distance-based lighting (objects fade with distance)
	litColor := rh.game.threading.ColorCalculator.CalculateDistanceColor(
		baseColor, distance, rh.game.camera.ViewDist, rh.game.config.Graphics.BrightnessMin)

	// Combine side-based shading with distance lighting
	finalColor := color.RGBA{
		R: uint8(float64(litColor.R) * shadingMultiplier),
		G: uint8(float64(litColor.G) * shadingMultiplier),
		B: uint8(float64(litColor.B) * shadingMultiplier),
		A: litColor.A,
	}
...
// CalculateSpriteMetrics calculates sprite position and size for 3D rendering (for monsters)
func (rh *RenderingHelper) CalculateSpriteMetrics(entityX, entityY, distance float64) (screenX, screenY, spriteSize int, visible bool) {
	return rh.calculateSpriteMetricsWithConfig(entityX, entityY, distance, rh.game.config.Graphics.Monster.MaxSpriteSize, rh.game.config.Graphics.Monster.MinSpriteSize, rh.game.config.Graphics.Monster.SizeDistanceMultiplier)
}
...
func (rh *RenderingHelper) CalculateNPCSpriteMetrics(entityX, entityY, distance float64) (screenX, screenY, spriteSize int, visible bool) {
	...
	// Adjust Y position to place NPCs on the ground (lower half of screen)
	if visible {
		screenY = rh.game.config.GetScreenHeight()/2 + spriteSize/4 - spriteSize/2
	}
...
func (rh *RenderingHelper) calculateSpriteMetricsWithConfig(entityX, entityY, distance float64, maxSize, minSize, multiplier int) (screenX, screenY, spriteSize int, visible bool) {
	...
	if distance > rh.game.camera.ViewDist || distance < 10 {
		return 0, 0, 0, false
	}
