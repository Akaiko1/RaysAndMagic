...
func TestCombatBalance_Save1PartyJungleAndCliffs(t *testing.T) {
	...
	for _, mk := range []string{"dragon", "dragon_red", "dragon_green", "dragon_gold", "gorilla_titan", "orc_hero_boss"} {
		...
		// Dragon Breath is a separate DragonBreathChance proc: a whole-party hit of
		// DragonBreathDamageType (respects IgnoresArmor, +TrueDamage). NOT tied to
		// the projectile spell - it shows for green dragons (no projectile) too, and
		// uses the breath's real element, not the bolt's (dragon_red breathes dark
		// while its bolt is fire).
		if ref.DragonBreathChance > 0 {
			row4(fmt.Sprintf("  breath:%s (%.0f%% party)", ref.DragonBreathDamageType, ref.DragonBreathChance*100),
				ref.DragonBreathDamageType, ref.IgnoresArmor, attackRoll)
		}
		// The non-breath attack: a projectile dragon fires its bolt (never melees);
		// everyone else melees.
		if ref.ProjectileSpell != "" {
			school := "?"
			if d, ok := config.GetSpellDefinition(ref.ProjectileSpell); ok && d != nil {
				school = d.School
			}
			row4("  ranged:"+ref.ProjectileSpell+" ("+school+")", school, false, attackRoll)
		} else {
