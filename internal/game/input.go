...
func (ih *InputHandler) handleNPCInteraction() {
	...
	for _, npc := range ih.game.world.NPCs {
...
func (ih *InputHandler) addSpellToCharacter(char *character.MMCharacter, spellData *character.NPCSpell) {
	...
	case "fire":
		targetSchool = character.MagicFire
	case "water":
		targetSchool = character.MagicWater
	case "air":
		targetSchool = character.MagicAir
	case "earth":
		targetSchool = character.MagicEarth
	case "body":
		targetSchool = character.MagicBody
	case "mind":
		targetSchool = character.MagicMind
	case "spirit":
		targetSchool = character.MagicSpirit
	case "light":
		targetSchool = character.MagicLight
	case "dark":
		targetSchool = character.MagicDark
	default:
		targetSchool = character.MagicWater // Default to water school
	}
	...
	if char.MagicSchools[targetSchool] == nil {
		char.MagicSchools[targetSchool] = &character.MagicSkill{
			...
			Spells:  make([]character.Spell, 0),
		}
	}

	// Add the spell to the school
	newSpell := character.Spell{
		Name:        spellData.Name,
		School:      targetSchool,
		Level:       spellData.Level,
		SpellPoints: spellData.SpellPoints,
	}

	char.MagicSchools[targetSchool].Spells = append(char.MagicSchools[targetSchool].Spells, newSpell)
}
