...
func NewMMGame(cfg *config.Config) *MMGame {
	...
	world := world.NewWorld3D(cfg)

	// Create a 4-character party
	party := character.NewParty(cfg)

	// Start in elvish forest - get starting position from loaded map
	...
	// Cache sky and ground images
	skyImg := ebiten.NewImage(cfg.GetScreenWidth(), cfg.GetScreenHeight()/2)
	skyColor := cfg.Graphics.Colors.Sky
	skyImg.Fill(color.RGBA{uint8(skyColor[0]), uint8(skyColor[1]), uint8(skyColor[2]), 255})

	groundImg := ebiten.NewImage(cfg.GetScreenWidth(), cfg.GetScreenHeight()/2)
	groundColor := cfg.Graphics.Colors.Ground
	groundImg.Fill(color.RGBA{uint8(groundColor[0]), uint8(groundColor[1]), uint8(groundColor[2]), 255})
	...
	game.collisionSystem = collision.NewCollisionSystem(world, float64(cfg.World.TileSize))
	...
	world.RegisterMonstersWithCollisionSystem(game.collisionSystem)
