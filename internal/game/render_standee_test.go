...
// comes closer than minRad to the camera's sight line (slabs have period π).
func TestClampYawFromEdgeOn(t *testing.T) {
	...
	// Slab period π: a yaw offset by ~π from the sight line is still edge-on.
...
func TestApproachAngle(t *testing.T) {
	...
	// Shortest arc across the ±π seam: from 3.0 to -3.0 is +0.28 rad forward,
	// not -6.0 backward.
