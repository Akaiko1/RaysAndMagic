...
func (r *Renderer) precomputeFloorColorCache() {
	defaultGreen := color.RGBA{60, 180, 60, 255}
	defaultDarkGreen := color.RGBA{20, 90, 20, 255}
	cache := make(map[[2]int]color.RGBA)

	// Estimate world bounds
	worldWidth, worldHeight := r.game.world.Width, r.game.world.Height
	...
	for tileY := 0; tileY < worldHeight; tileY++ {
		for tileX := 0; tileX < worldWidth; tileX++ {
			...
			t := r.game.world.GetTileAt(checkX, checkY)
	...
	for tileY := 0; tileY < worldHeight; tileY++ {
		for tileX := 0; tileX < worldWidth; tileX++ {
			...
			checkX := float64(tileX)*float64(r.game.config.GetTileSize()) + float64(r.game.config.GetTileSize())/2
			checkY := float64(tileY)*float64(r.game.config.GetTileSize()) + float64(r.game.config.GetTileSize())/2
			currentTile := r.game.world.GetTileAt(checkX, checkY)
	...
	if r.game.world != nil {
		spawnTileX := r.game.world.StartX
		spawnTileY := r.game.world.StartY
...
func (r *Renderer) performMultiHitRaycast(angle float64) MultiRaycastHit {
	...
	for steps := 0; steps < maxSteps; steps++ {
		...
		tileType := r.game.world.GetTileAt(worldX, worldY)
...
func (r *Renderer) drawMonstersParallel(screen *ebiten.Image) {
	...
	for _, monster := range r.game.world.Monsters {
		if !monster.IsAlive() {
			continue
		}
...
func (r *Renderer) drawNPCs(screen *ebiten.Image) {
	...
	for _, npc := range r.game.world.NPCs {
		// Calculate distance - use squared distance for initial culling to avoid sqrt
		dx := npc.X - camX
		dy := npc.Y - camY
		distanceSq := dx*dx + dy*dy
...
func (r *Renderer) drawMagicProjectiles(screen *ebiten.Image) {
	for _, fireball := range r.game.fireballs {
		...
		// Calculate fireball size based on distance using proper 3D perspective
		baseSize := float64(r.game.config.Graphics.Projectiles.Fireball.BaseSize)
		fireballSize := int(baseSize / distance * r.game.config.GetTileSize())
		if fireballSize > r.game.config.Graphics.Projectiles.Fireball.MaxSize {
			fireballSize = r.game.config.Graphics.Projectiles.Fireball.MaxSize
		}
		if fireballSize < r.game.config.Graphics.Projectiles.Fireball.MinSize {
			fireballSize = r.game.config.Graphics.Projectiles.Fireball.MinSize
		}

		screenY := r.game.config.GetScreenHeight()/2 - fireballSize/2

		// Draw projectile with VERY distinct colors and sizes based on spell type
		var projectileColor [3]int
		var adjustedSize int = fireballSize

		switch fireball.SpellType {
		case "SpellTypeFireBolt":
			projectileColor = [3]int{255, 255, 100} // Bright yellow-orange for Fire Bolt
			adjustedSize = fireballSize * 5 / 10    // 50% smaller (much more noticeable)
		case "SpellTypeFireball":
			projectileColor = [3]int{255, 50, 0}  // Deep red for Fireball
			adjustedSize = fireballSize * 15 / 10 // 50% larger (much bigger)
		case "SpellTypeLightning":
			projectileColor = [3]int{100, 150, 255} // Bright blue for Lightning
			adjustedSize = fireballSize * 6 / 10    // 40% smaller than fireball
		default:
			// Default to fireball color for unknown types
			projectileColor = r.game.config.Graphics.Projectiles.Fireball.Color
			adjustedSize = fireballSize
		}

		fireballImg := ebiten.NewImage(adjustedSize, adjustedSize)
		fireballImg.Fill(color.RGBA{uint8(projectileColor[0]), uint8(projectileColor[1]), uint8(projectileColor[2]), 255})

		opts := &ebiten.DrawImageOptions{}
		opts.GeoM.Translate(float64(screenX-adjustedSize/2), float64(screenY))
...
func (r *Renderer) drawSwordAttacks(screen *ebiten.Image) {
	for _, attack := range r.game.swordAttacks {
		...
		// Calculate attack size based on distance using proper 3D perspective
		baseSize := float64(r.game.config.Graphics.Projectiles.SwordAttack.BaseSize)
		attackSize := int(baseSize / distance * r.game.config.GetTileSize())
		if attackSize > r.game.config.Graphics.Projectiles.SwordAttack.MaxSize {
			attackSize = r.game.config.Graphics.Projectiles.SwordAttack.MaxSize
		}
		if attackSize < r.game.config.Graphics.Projectiles.SwordAttack.MinSize {
			attackSize = r.game.config.Graphics.Projectiles.SwordAttack.MinSize
		}

		screenY := r.game.config.GetScreenHeight()/2 - attackSize/2

		// Draw sword attack as a silver/white projectile
		attackImg := ebiten.NewImage(attackSize, attackSize)
		attackColor := r.game.config.Graphics.Projectiles.SwordAttack.Color
...
func (r *Renderer) drawArrows(screen *ebiten.Image) {
	for _, arrow := range r.game.arrows {
		...
		// Calculate arrow size based on distance using proper 3D perspective
		baseSize := float64(r.game.config.Graphics.Projectiles.SwordAttack.BaseSize) * 0.7 // Smaller than sword attacks
		arrowSize := int(baseSize / distance * r.game.config.GetTileSize())
		if arrowSize > r.game.config.Graphics.Projectiles.SwordAttack.MaxSize-5 {
			arrowSize = r.game.config.Graphics.Projectiles.SwordAttack.MaxSize - 5
		}
		if arrowSize < r.game.config.Graphics.Projectiles.SwordAttack.MinSize {
			arrowSize = r.game.config.Graphics.Projectiles.SwordAttack.MinSize
		}

		screenY := r.game.config.GetScreenHeight()/2 - arrowSize/2

		// Draw arrow as a brown/wooden projectile
		arrowImg := ebiten.NewImage(arrowSize, arrowSize)
		// Brown color for wooden arrow
		arrowImg.Fill(color.RGBA{139, 69, 19, 255}) // Saddle brown color
