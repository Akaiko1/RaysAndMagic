...
func (cs *CombatSystem) ApplyDamageToMonster(monster *monsterPkg.Monster3D, damage int, weaponName string) {
	...
	// Add combat message
	if monster.IsAlive() {
		cs.game.AddCombatMessage(fmt.Sprintf("%s hits %s with %s for %d damage!", 
			cs.game.party.Members[cs.game.selectedChar].Name, monster.Name, weaponName, finalDamage))
	} else {
		cs.game.AddCombatMessage(fmt.Sprintf("%s kills %s with %s for %d damage!", 
			cs.game.party.Members[cs.game.selectedChar].Name, monster.Name, weaponName, finalDamage))
		
		// Award experience and gold
		for _, member := range cs.game.party.Members {
			member.Experience += monster.Experience / len(cs.game.party.Members)
		}
...
func (cs *CombatSystem) handleProjectileMonsterCollision(projectileEntity, monsterEntity *collision.Entity) {
	...
	case "magic_projectile":
		...
		spellName := magicProjectile.SpellType
		if spellDef, err := spells.GetSpellDefinitionByID(spells.SpellID(spellName)); err == nil && spellDef.Name != "" {
			spellName = spellDef.Name
		}

		// Monster takes fire damage with distance-aware AI response
		actualDamage := monster.TakeDamage(magicProjectile.Damage, monsterPkg.DamageFire, cs.game.camera.X, cs.game.camera.Y)
