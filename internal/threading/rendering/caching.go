...
// This struct defines all the parameters that affect the final appearance of a rendered wall slice.
// Distance and texture coordinates are quantized during caching to improve hit rates.
type WallSliceKey struct {
	...
	Distance float64     // Distance from camera (quantized to multiples of 10 for caching)
	Side     int         // Wall orientation: 0 for north-south walls, 1 for east-west walls (affects shading)
...
func (wsc *WallSliceCache) GetOrCreate(key WallSliceKey, createFunc func() *ebiten.Image) *ebiten.Image {
	// Quantize distance and texture coordinate for better cache hit rates
	// Distance is rounded to nearest 10 units to group similar distances together
	key.Distance = float64(int(key.Distance/10)) * 10
